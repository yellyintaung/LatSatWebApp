            ...
            <div class="col-md-3">
                ...
                <a href="/product_detail/{{ $item->id }}" class="img-zoom"><figure class="m-0"><img src="/uploads/{{ $item->product_img }}" class="card-img-top" style="width: 100%;height: 180px;" alt="..."></figure></a> 
                    <div class="card-body" style="padding-top: 0.7rem;">
                        ...
                        <p class="card-text m-0 pb-1 pt-1"><small>၁{{ $item->type->type_name }} - {{$item->price }}ကျပ်</small></p>
                        
                        <h6 class="default-color">အရေအတွက်</h6>
                        <div class="wrap w-100 pt-1">
                            <button type="button" id="sub" class="sub">-</button>
                            <input class="count pl-2 txt-qy" id="count" type="text" value="1" min="1" max="1000" style="width:42%;" /> {{ $item->type->type_name }}
                            <button type="button" id="plus" class="plus">+</button>
                        </div>
                        <a class="btn btn-sm btn-add add text-white mt-3 float-right" style="cursor:pointer;" data-id="{{ $item->id }}">Add To Cart</a>
                    </div>
                </div>
            </div>
            @endforeach
